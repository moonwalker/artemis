...
export default ({ owner, repo, branch, path }) => {
    ...
    const save = (e) => {
        ...
        client.post(endpoints.blob(owner, repo, branch, path), { contents: btoa(payload) }).then(res => {
            if (res.error)
                ...
                <div className="file-list">
                    ...
                    {!!content && <div className="w-full px-4">
                        <div className="rounded-md rounded-t-none border border-gray-300 text-gray-70 mb-4">
                            <textarea className="block h-96 py-4 px-3 w-full text-sm text-gray-700 placeholder-gray-500 font-medium outline-none bg-transparent border border-gray-400 hover:border-white focus:border-green-500 rounded-lg resize-none" id="content-editor" type="text" defaultValue={content} >
                            </textarea>
                        </div>
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-2">
                                <button className="border border-gray-400 rounded-md px-4 py-1 hover:bg-gray-200 flex items-center space-x-2" onClick={save}>
                            ...
                            <div className="flex items-center space-x-2">
                                <button className="group border border-green-400 text-white bg-green-600 rounded-md px-4 py-1 hover:bg-green-200 hover:text-gray-600 flex items-center space-x-2" onClick={cancel}>
