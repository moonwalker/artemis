...
export default ({ owner, repo, branch, path}) => {  
    ...
    const formatItemUrl = (i) => {
        let url = `/${owner}/${repo}/${i.type}/${branch}`
        if (!!path) {
            url += `/${path}`
        }
        url += `/${i.name}`
                ...
                <div className="file-explorer rounded-md rounded-t-none border border-gray-300 text-gray-700 divide-y divide-gray-300">
                    ...
                    {!!items && items.map((i, idx) => (<Link key={idx} to={i.url} className="flex justify-between px-4 py-2 hover:bg-gray-200">
                        <div className="w-4/12 flex items-center">
                            {i.type == 'tree' && <TreeIcon />}
                            {i.type == 'blob' && <BlobIcon />}
...
const TreeIcon = () => (<svg className="w-4 fill-current text-blue-400" color="blue-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
...
const BlobIcon = () => (<svg className="w-4 fill-current text-gray-600" color="gray-light" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
